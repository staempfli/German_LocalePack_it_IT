 ...
 */
class German_LocalePackIt_Block_System_Config_Form_Fieldset_Magentoconnectmanager
    extends Mage_Adminhtml_Block_System_Config_Form_Fieldset
{
    public function render(Varien_Data_Form_Element_Abstract $element)
    {
        $helper = Mage::helper("localepackit");
        ...
        $html .= '<li>"template_it" '.$helper->__("or").'</li>';
        $html .= '<li>"'.$helper->__("from").' Magento 1.5.x "template_it_1.5" '.$helper->__("or").'</li>';
        $html .= '<li>"'.$helper->__("from").' Magento 1.7.x "template_it_1.7"</li></ul>';
        $html .= $helper->__('rename into "template"').'.</li>';
        $html .= "</ol>";
        $html .= "<p>".$helper->__("The Magento Connect Manager should be changed to Italian now.")."</p>";
